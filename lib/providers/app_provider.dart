...
// Sync
  ...
  buffer.writeln('/*');
  buffer.writeln(' * $appName - Arduino/ESP Device Controller (MQTT)');
  ...
  buffer.writeln('  if (!client.connected()) { reconnect(); }');
  buffer.writeln('  client.loop();');
  buffer.writeln('}');
  buffer.writeln();

  buffer.writeln('void callback(char* topic, byte* payload, unsigned int length) {');
  buffer.writeln('  String message;');
  buffer.writeln('  for (int i = 0; i < length; i++) { message += (char)payload[i]; }');
  buffer.writeln('  Serial.print("Message arrived ["); Serial.print(topic); Serial.print("] "); Serial.println(message);');
  buffer.writeln();

  for (int i = 0; i < _devices.length; i++) {
    final device = _devices[i];
    if (device.gpioPin != null) {
      buffer.writeln('  if (String(topic) == "home/${device.id}/set") {');
      buffer.writeln('    if (message == "ON") {');
      buffer.writeln('      digitalWrite(CONTROL_PIN_${device.name.toUpperCase().replaceAll(' ', '_').replaceAll(RegExp(r'[^A-Z0-9_]'), '')}, RELAY_ON);');
      buffer.writeln('      client.publish("home/${device.id}/status", "ON");');
      buffer.writeln('    } else {');
      buffer.writeln('      digitalWrite(CONTROL_PIN_${device.name.toUpperCase().replaceAll(' ', '_').replaceAll(RegExp(r'[^A-Z0-9_]'), '')}, RELAY_OFF);');
      buffer.writeln('      client.publish("home/${device.id}/status", "OFF");');
      buffer.writeln('    }');
      buffer.writeln('  }');
    }
  }
  ...
  buffer.writeln('    Serial.print("Attempting MQTT connection...");');
  buffer.writeln('    if (client.connect("arduino_client")) {');
  buffer.writeln('      Serial.println("connected");');
  for (int i = 0; i < _devices.length; i++) {
    final device = _devices[i];
    buffer.writeln('      client.subscribe("home/${device.id}/set");');
  }
  buffer.writeln('    } else {');
  buffer.writeln('      Serial.print("failed, rc=");');
  buffer.writeln('      Serial.print(client.state());');
  buffer.writeln('      Serial.println(" try again in 5 seconds");');
